...
function updateValue(e) {
    ...
    //alert("full path alert" + fullPath);
    if (fullPath) {
        var startIndex = (fullPath.indexOf('\\') >= 0 ? fullPath.lastIndexOf('\\') : fullPath.lastIndexOf('/'));
        var filename = fullPath.substring(startIndex);
        var filename = filename.slice(0, filename.lastIndexOf(".")) //removing evrithing after . like .mp4
        if (filename.indexOf('\\') === 0 || filename.indexOf('/') === 0) {
            filename = filename.substring(1);
            filename = filename.concat("Output.json"); // Adding ".json" suffix to the name
        }
        //alert("file name and calling get json" + filename);
        getJson(filename);
        //load_json_from_url(document.getElementById(filename));
                        }
        }
...
function load_video_from_url(url) {
    video.src = url
    //alert("Inside load video from url calling json")
    //getJson()
}
...
load_json_from_url("FriendsOutput.json")
load_video_from_url("assets/test_video.mp4")

const bucketName = 'vid-int-demo';
const fileName = 'FriendsOutput.json';
const destFileName = 'vid-int-demo/FriendsOutput.json';
